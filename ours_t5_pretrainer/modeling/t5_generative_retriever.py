...
class T5ForDocIDGeneration(T5PreTrainedModel):
    ...
    def get_decoder_inputs_embeds(self, decoder_input_ids: Optional[torch.LongTensor]):
        """
        case 1: [[-1], [-1]] # batch size 2, 두개 다 start_token 사용
        case 2: [[-1, 24], [-1, 0]] # batch size 2,[[start_token, 24번 토큰], [start_token, 0번 토큰]]
        """
        input_shape = decoder_input_ids.size() 
        batch_size, seq_length = input_shape
        assert seq_length <= self.num_decoder_embeds

        start_token_embeds = self.start_token_embed.expand(batch_size, 1, -1) # [batch_size, 1, d_model]
        ...
        if seq_length == 1:
            ...
            for i in range(1, seq_length):
                embeds = self.list_decoder_embeds[i-1](decoder_input_ids[:, i]).unsqueeze(1) # [batch_size, 1, d_model]
                input_embeds.append(embeds)
            input_embeds = torch.cat(input_embeds, dim=1)

        return input_embeds

    # 2개 이상의 토큰의 임베딩을 평균내어서 사용
    def get_decoder_mul_inputs_embeds(self, decoder_input_ids: Optional[torch.LongTensor]):
        ...
        Args:
            ...
            case 2: [
                [[-1, -1], [2, 4]], -> 2번, 4번 토큰의 임베딩을 평균 내어서 사용
                [[-1, -1], [3, 1]], -> 3번, 1번 토큰의 임베딩을 평균 내어서 사용
                    ]
        Returns:
            [bz, seq_len, d_model]
        """
