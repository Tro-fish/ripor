...
class T5ForDocIDGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5forDocIDConfig):
        ...
        self.decoder_embedding = nn.Embedding(config.decoder_vocab_size, config.d_model)
        ...
        # 입력, 출력 임베딩을 공유하지 않을 경우
        if not config.shared_output_input_embeds:
            self.decoder_output_embed = nn.Embedding(config.decoder_vocab_size, config.d_model)
    ...
    def get_decoder_inputs_embeds(self, decoder_input_ids: Optional[torch.LongTensor]):
        """
        case 1: [[-1], [-1]] -> 그냥 start token만을 사용할 때
        case 2: [[-1, 24], [-1, 0]] -> start token 이후로도 계속 생성을 할 때
        """
        ...
        if seq_length == 1:
            ...
            for i in range(1, seq_length):
                embeds = self.decoder_embedding(decoder_input_ids[:, i]).unsqueeze(1) #[bz, 1, d_model]
    ...
    def get_decoder_mul_inputs_embeds(self, decoder_input_ids: Optional[torch.LongTensor]):
        ...
        if seq_length == 1:
            ...
            input_embeds = [start_token_embeds]
            for i in range(1, seq_length):
                embed_layer = self.decoder_embedding
    ...
    def get_lm_logits(self, sequence_output):
        ...
        for i in range(sequence_output.size(1)):
            if self.config.shared_output_input_embeds:
                logits = (sequence_output[:, i, :] @ self.decoder_embedding.weight.t())
            else:
                logits = (sequence_output[:, i, :] @ self.decoder_embedding.weight.t())
    ...
    def assign_decoder_embed(self, path, idx):
        raise Exception("Not implemented: assign_decoder_embed()")
    ...
    def resize_and_assign_decoder_embed(self, path, idx):
        raise Exception("Not implemented: resize_and_assign_decoder_embed()")
...
class T5SeqPretrainEncoder(T5DocIDGenEncoder):
    ...
    def get_commit_loss(self,
        ...
        if pos_prev_smtids.dim() == 2:
            ...
            hard_label = False
            vocab_size = self.base_model.decoder_embedding.weight.size(0)
        ...
        for reps, labels in zip(all_reps, all_labels):
            ...
            for i in range(reps.size(1)):
                embed_weight = self.base_model.decoder_embedding.weight
